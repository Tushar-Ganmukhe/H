  ...
  const handleSend = async (customMessage = null) => {
    ...
    try {
      const res = await axios.post(`${API_BASE}/chat`, {
        ...
        // Use user's mobile number as unique session ID for memory
  ...
  return (
    <div className="flex flex-col h-screen bg-[#F8FAFC]">
      ...
      <header className="bg-white border-b border-gray-200 px-8 py-4 flex justify-between items-center shadow-sm">
        ...
        {/* Added Logout Button next to Trash icon */}
        <div className="flex gap-4">
          <button onClick={() => setMessages([messages[0]])} className="p-2.5 text-gray-400 hover:text-red-500 hover:bg-red-50 rounded-xl transition-all" title="Clear Chat">
            <Trash2 size={20} />
          </button>
          <button onClick={handleLogout} className="flex items-center gap-2 px-4 py-2 bg-red-50 text-red-600 rounded-xl font-semibold hover:bg-red-100 transition-all">
