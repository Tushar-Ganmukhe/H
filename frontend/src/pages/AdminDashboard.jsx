...
const translations = {
  en: {
    portal: "Admin Portal", overview: "Overview", inventory: "Inventory", analytics: "Analytics",
    health: "System Health.", welcome: "Welcome, Admin. Database & AI Agents are Synced.",
    stats: ["Active Inventory", "7d Revenue", "Low Stock"],
    ...
    empty: "No products found in database. Please upload Excel.", loading: "Querying Database..."
  },
  hi: {
    portal: "एडमिन पोर्टल", overview: "अवलोकन", inventory: "इन्वेंटरी", analytics: "विश्लेषण",
    health: "सिस्टम स्वास्थ्य।", welcome: "नमस्ते एडमिन। डेटाबेस और एआई एजेंट सिंक्रनाइज़ हैं।",
    stats: ["सक्रिय सूची", "7दिन का राजस्व", "कम स्टॉक"],
    ...
    empty: "डेटाबेस में कोई उत्पाद नहीं मिला। कृपया एक्सेल अपलोड करें।", loading: "डेटाबेस खोजा जा रहा है..."
  },
  mr: {
    portal: "अ‍ॅडमिन पोर्टल", overview: "आढावा", inventory: "इन्व्हेंटरी", analytics: "विश्लेषण",
    health: "सिस्टम आरोग्य।", welcome: "नमस्ते अ‍ॅडमिन. डेटाबेस आणि एआय एजंट सिंक्रोनाइझ झाले आहेत.",
    stats: ["सक्रिय इन्व्हेंटरी", "7दिवसांचा महसूल", "कमी स्टॉक"],
    ...
    empty: "डेटाबेसमध्ये उत्पादने आढळली नाहीत. कृपया एक्सेल अपलोड करा.", loading: "डेटाबेस तपासत आहे..."
  }
};
...
const AdminDashboard = () => {
  ...
  // 1. FETCH DATA ON MOUNT (This solves the "Empty on Refresh" issue)
  useEffect(() => {
    refreshAllData();
  }, []);
  ...
  const handleBulkUpload = async (e) => {
    ...
    try {
      ...
      setUploadStatus({ 
        type: "success", 
        msg: `✅ Sync Done! Added: ${res.data.added}, Updated: ${res.data.updated}` 
      });
  ...
  const handleSaveEdit = async (id) => {
    try {
      ...
      setEditingId(null);
      await fetchInventory(); // Re-fetch to confirm save
        ...
        <nav className="flex bg-gray-100 p-1.5 rounded-2xl gap-2 border border-gray-200">
          {["overview", "inventory", "analytics"].map((tab) => (
            <button key={tab} onClick={() => setActiveTab(tab)}
              className={`px-8 py-2.5 rounded-xl text-xs font-black uppercase tracking-widest transition-all ${activeTab === tab ? "bg-white text-indigo-600 shadow-md scale-105" : "text-gray-400"}`}>
              {t[tab]}
            </button>
          ))}
        </nav>
      ...
      <main className="flex-1 overflow-y-auto p-10">
        ...
        {activeTab === "overview" && (
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
              ...
              <StatCard icon={<AlertCircle/>} label={t.stats[2]} value={inventory.filter(i => i.stock < 10).length} color="orange" />
            </div>
          </div>
        )}
        ...
        {activeTab === "inventory" && (
          <div className="bg-white rounded-[3rem] border border-gray-200 shadow-2xl overflow-hidden min-h-[600px] animate-in slide-in-from-bottom-5">
            ...
            {loading ? (
              <div className="p-32 text-center text-gray-300 font-black text-2xl animate-pulse">{t.loading}</div>
            ) : (
              <table className="w-full text-left">
                <thead className="bg-gray-50 text-gray-400 text-[10px] uppercase font-black tracking-[0.2em] border-b border-gray-100">
                  <tr>
                    <th className="px-10 py-6">{t.table[0]}</th>
                    <th className="px-10 py-6">{t.table[1]}</th>
                    <th className="px-10 py-6">{t.table[2]}</th>
                    <th className="px-10 py-6">{t.table[3]}</th>
                    <th className="px-10 py-6">{t.table[4]}</th>
                    <th className="px-10 py-6 text-right">{t.table[5]}</th>
                  </tr>
                </thead>
                  ...
                  {inventory.map((item) => (
                    <tr key={item.product_id} className="hover:bg-gray-50/50 transition-all group">
                      ...
                      {editingId === item.product_id ? (
                        <>
                          <td className="px-10 py-8"><input type="number" className="w-24 border-2 border-indigo-100 rounded-xl p-2 font-bold" value={editForm.stock} onChange={e=>setEditForm({...editForm, stock: e.target.value})}/></td>
                          <td className="px-10 py-8"><input type="number" className="w-24 border-2 border-indigo-100 rounded-xl p-2 font-bold" value={editForm.price} onChange={e=>setEditForm({...editForm, price: e.target.value})}/></td>
                          <td className="px-10 py-8">
                            <select className="border-2 border-indigo-100 rounded-xl p-2 font-bold" value={editForm.prescription_required} onChange={e=>setEditForm({...editForm, prescription_required: e.target.value === 'true'})}>
                              <option value="true">{t.rx}</option>
                              <option value="false">{t.otc}</option>
                            </select>
                          </td>
                          <td className="px-10 py-8 text-right flex justify-end gap-2 mt-4 mr-4">
                            <button onClick={() => handleSaveEdit(item.product_id)} className="bg-green-500 text-white p-3 rounded-xl shadow-lg hover:bg-green-600 transition-all"><Save size={18}/></button>
                            <button onClick={() => setEditingId(null)} className="bg-gray-200 text-gray-400 p-3 rounded-xl hover:bg-gray-300 transition-all"><X size={18}/></button>
                          </td>
                        </>
                      ) : (
                        <>
                          <td className="px-10 py-8">
                            <span className={`px-5 py-2 rounded-full text-[10px] font-black uppercase tracking-widest border ${item.stock < 10 ? 'bg-red-50 text-red-600 border-red-100' : 'bg-green-50 text-green-600 border-green-100'}`}>{item.stock} Units</span>
                          </td>
                          ...
                          <td className="px-10 py-8 text-right opacity-0 group-hover:opacity-100 transition-opacity">
                            <button onClick={() => { setEditingId(item.product_id); setEditForm(item); }} className="p-4 bg-indigo-50 text-indigo-600 rounded-2xl hover:bg-indigo-600 hover:text-white transition-all transform hover:scale-110"><Edit2 size={22}/></button>
                          </td>
                        </>
                      )}
                    </tr>
            ...
            {inventory.length === 0 && !loading && (
              <div className="p-32 text-center text-gray-300 font-black text-2xl">{t.empty}</div>
            )}
          </div>
        )}

        {activeTab === "analytics" && (
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10 h-[600px] animate-in zoom-in duration-500">
...
const StatCard = ({ icon, label, value, color }) => (
  <div className="bg-white p-10 rounded-[3rem] border border-gray-100 shadow-xl flex items-center gap-8 group hover:scale-105 transition-transform duration-300 cursor-default">
